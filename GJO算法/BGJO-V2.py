...
class Test:
    ...
    def transform(self, Position, dim):  # 该函数是对位置进行二进制映射
        ...
        for j in range(Position.shape[0]):  # 列数
            v = math.fabs(math.tanh(Position[j]))
            if v > random.random():
                if Position_B[j] == 1:
                    Position_B[j] = 0
                else:
    ...
    # 变异操作
    def bianyi(self, X2, iter, Max_iter):
        for i in range(self.data.nb_attribs):
            eta = 0.8-0.6/(1+math.exp(5-10*iter/Max_iter))
    ...
    # 选择解
    def tubian(self, X1, X2, iter, Max_iter):
        eta = (0.5+0.05)-(2*0.05)/(1+math.exp(5-10*iter/Max_iter))
        if random.random() < eta:
            return X1
        else:
            return X2
    ...
    # 交叉操作
    def jiaocha(self, X_Old, X_New):
        X_B = X_Old * 1
        for j in range(X_Old.shape[0]):  # 列数
    ...
    def BGJO(self, pop, Max_iteration):
        ...
        for t in range(Max_iteration):
            ...
            for i in range(int(pop/2), pop):
                X_cm = np.array(np.zeros(dim, int))
                subs = random.random()
                if subs < 0.33:
                    X_cm = self.bianyi(Male_Jackal, t, Max_iteration)
                elif subs < 0.66:
                    X_cm = self.bianyi(Female_Jackal, t, Max_iteration)
                else:
                    X_random = self.jiaocha(Male_Jackal, Female_Jackal)
                    X_cm = self.bianyi(X_random, t, Max_iteration)
...
class FSData:
    ...
    def __init__(self, location, nbr_exec, particle_num, max_iter):
        ...
        self.instance_name = str(
            "BGJO-V2_" + time.strftime("%m-%d-%H-%M_", time.localtime()))  # 例子的名字
