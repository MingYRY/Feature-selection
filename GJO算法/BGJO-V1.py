...
class Test:
    ...
    def transform(self, Position, dim):  # 该函数是对位置进行二进制映射
        ...
        for j in range(Position.shape[0]):  # 列数
            v = math.fabs(math.erf((math.sqrt(math.pi)/2) * Position[j]))
            if v > random.random():
                if Position_B[j] == 1:
                    Position_B[j] = 0
                else:
        ...
        for t in range(Max_iteration):
            ...
            for i in range(pop):
                for j in range(dim):
                    ...
                    eta = 3/2 - 1.0 / (1 + math.exp(5 - 10 * t / Max_iteration))
                    if abs(E) < 1:
                        # EXPLOITATION
                        D_male_jackal = abs((eta*RL[i, j] * Male_Jackal[j] - X[i, j]))
                        Male_Positions[i, j] = Male_Jackal[j] - E * D_male_jackal
                        D_female_jackal = abs((eta*RL[i, j] * Female_Jackal[j] - X[i, j]))
                        Female_Positions[i, j] = Female_Jackal[j] - E * D_female_jackal
                    else:
                        # EXPLORATION
                        D_male_jackal = abs((Male_Jackal[j] - eta*RL[i, j] * X[i, j]))
                        Male_Positions[i, j] = Male_Jackal[j] - E * D_male_jackal
                        D_female_jackal = abs((Female_Jackal[j] - eta*RL[i, j] * X[i, j]))
                        Female_Positions[i, j] = Female_Jackal[j] - E * D_female_jackal
...
class FSData:
    ...
    def __init__(self, location, nbr_exec, particle_num, max_iter):
        ...
        self.instance_name = str(
            "BGJO-V1_" + time.strftime("%m-%d-%H-%M_", time.localtime()))  # 例子的名字
