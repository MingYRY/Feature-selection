...
class Test:
    ...
    def BHHO(self, pop, Max_iteration):
        print("The BHHO algorithm is optimizing your problem...")
        ...
        Rabbit_Location = np.array(np.zeros(dim, int))
        ...
        for t in range(Max_iteration):
            ...
            for i in range(pop):
                if Rabbit_Energy > fitness[i]:  # 更新全局最优
                    Rabbit_Energy = copy.copy(fitness[i])
                    Rabbit_Location = copy.copy(X[i, :])

            food_x[t] = Rabbit_Energy  # 保存每次迭代的最小的适应度函数值

            E1 = 2 * (1 - (t / Max_iteration))
            # Update the location of Harris' hawks
            for i in range(0, pop):
                E0 = 2 * random.random() - 1  # -1<E0<1
                Escaping_Energy = E1 * E0  # escaping energy of rabbit Eq. (3) in the paper

                # -------- 全局搜索阶段 Eq. (1) in paper -------------------

                if abs(Escaping_Energy) >= 1:
                    # Harris' hawks perch randomly based on 2 strategy:
                    q = random.random()
                    rand_Hawk_index = math.floor(pop * random.random())  # 随机选择一只鹰
                ...
                # -------- 局部开发阶段 -------------------
                    ...
                    if r < 0.5 and abs(Escaping_Energy) >= 0.5:  # Soft besiege Eq. (10) in paper
                        ...
                        # 越界判断
                        X1 = self.BorderCheck(X1, ub, lb, dim)
                        # 二进制映射
                        X1 = self.transform(X1, dim)
                        # 计算X1适应度函数值
                        fitness1 = self.CaculateFitness(X1)

                        if fitness1 < fitness:  # improved move?
                            X[i, :] = X1.copy()
                            ...
                            X2 = Rabbit_Location - Escaping_Energy * abs(
                                Jump_strength * Rabbit_Location - X[i, :]) + numpy.multiply(numpy.random.randn(dim),
                                                                                            self.Levy(dim))
                            # 越界判断
                            X2 = self.BorderCheck(X2, ub, lb, dim)
                            # 二进制映射
                            X2 = self.transform(X2, dim)
                            # 计算X2适应度函数值
                            fitness2 = self.CaculateFitness(X2)
                    ...
                    if r < 0.5 and abs(Escaping_Energy) < 0.5:  # Hard besiege Eq. (11) in paper
                        ...
                        if fitness1 < fitness:  # improved move?
                            ...
                            if fitness2 < fitness:
                                X[i, :] = X2.copy()
        print('gBest', Rabbit_Location)
        print('gBestScore', Rabbit_Energy)  # 迭代第200次之后的最小适应度函数值
        print('fitness', food_x)
        #       print(food_x)

        return self.data.evaluate(Rabbit_Location), self.nbrUn(Rabbit_Location), Rabbit_Energy, food_x
...
class FSData:
    ...
    def __init__(self, location, nbr_exec, particle_num, max_iter):
        ...
        self.instance_name = str(
            "BHHO_" + time.strftime("%m-%d-%H-%M_", time.localtime()))  # 例子的名字
    ...
    def run(self, alpha, beta):
        ...
        for itr in range(1, self.nb_exec + 1):
            ...
            best = test.BHHO(self.particle_num, self.max_iter)  # 粒子群数目，迭代次数
            t2 = time.time()
...
# Dataset
datasets = ['Breastcancer', 'BreastEW', 'HeartEW', 'Lymphography', 'sonar', 'Spect', 'Congress', 'Ionosphere', 'Krvskp',
            'Exactly', 'Exactly2', 'Vote', 'WaveformEW', 'Zoo', 'Lungcancer', 'WDBC']
