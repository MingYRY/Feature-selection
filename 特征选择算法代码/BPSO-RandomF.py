...
class Test:
    ...
    def BPSO(self, noP, Max_iteration):
        print("The BPSO algorithm is optimizing your problem...")
        noV = self.data.nb_attribs
        w = 2
        wMax = 0.9
        wMin = 0.4
        c1 = 1.5
        ...
        Velocity = np.array([[0 for i in range(noV)] for j in range(noP)])
        Position = np.array([[0 for i in range(noV)] for j in range(noP)])

        pBestScore = np.array(np.zeros(noP, float))
        pBest = np.array([[0 for i in range(noV)] for j in range(noP)])

        gBestScore = float("inf")
        gBest = np.array(np.zeros(noV, int))

        ConvergenceCurve = np.array(np.zeros(Max_iteration, int))
        ...
        for l in range(0, Max_iteration):
            ...
            for i in range(noP):
                for j in range(noV):
                    Velocity[i, j] = w * Velocity[i, j] + c1 * random.random() * (
                                pBest[i, j] - Position[i, j]) + c2 * random.random() * (gBest[j] - Position[i, j])
...
class FSData:
    ...
    def __init__(self, location, nbr_exec, particle_num, max_iter):
        ...
        self.instance_name = str(
            "BPSO-RandomF_" + time.strftime("%m-%d-%H-%M_", time.localtime()))  # 例子的名字
    ...
    def run(self, alpha, beta):
        ...
        for itr in range(1, self.nb_exec + 1):
            ...
            best = test.BPSO(self.particle_num, self.max_iter)  # 粒子群数目，迭代次数
            t2 = time.time()
...
# Dataset
datasets = ['train_TOM', 'train_Tumor', 'train_TWO']
