...
workbook = xlsxwriter.Workbook('C:/Users/dell/Desktop/data-cost/' + inputname + '_MOHHO9' + '.xlsx')
...
Rabbit_Energy = float("inf") * numpy.ones(3)  # change this to -inf for maximization problems

# Initialize the locations of Harris' hawks
population = numpy.random.randint(0, 2, (population_size, dim))  #

# Initialize convergence
convergence_curve = numpy.zeros(T)
...
#    for i in range(0, population_size):
    ...
    for i in range(0, population_size):
        ...
        if abs(Escaping_Energy) >= 1:
            ...
            q = random.random()
            rand_Hawk_index = math.floor(population_size * random.random())  # 随机选择一只鹰
        ...
        # -------- Exploitation phase -------------------
            ...
            if r < 0.5 and abs(Escaping_Energy) >= 0.5:  # Soft besiege Eq. (10) in paper
                ...
                X1 = transform(X1, dim)
                particles_X1 = single_get_ObjectFunction(X1, dim, x, y, z)
                if dominates(particles_X1, particles_F[i]):  # improved move?
                    population[i, :] = X1.copy()
                else:  # hawks perform levy-based short rapid dives around the rabbit
                    X2 = Rabbit_Location - Escaping_Energy * abs(
                        Jump_strength * Rabbit_Location - population[i, :]) + numpy.multiply(numpy.random.randn(dim),
                                                                                    Levy(dim))
                    X2 = transform(X2, dim)
                    particles_X2 = single_get_ObjectFunction(X2, dim, x, y, z)
                    if dominates(particles_X2, particles_F[i]):
                        population[i, :] = X2.copy()
            if r < 0.5 and abs(Escaping_Energy) < 0.5:  # Hard besiege Eq. (11) in paper
                Jump_strength = 2 * (1 - random.random())
                X1 = Rabbit_Location - Escaping_Energy * abs(Jump_strength * Rabbit_Location - population.mean(0))
                X1 = transform(X1, dim)
                particles_X1 = single_get_ObjectFunction(X1, dim, x, y, z)
                if dominates(particles_X1, particles_F[i]):  # improved move?
                    population[i, :] = X1.copy()
                    ...
                    X2 = Rabbit_Location - Escaping_Energy * abs(
                        Jump_strength * Rabbit_Location - population.mean(0)) + numpy.multiply(numpy.random.randn(dim),
                                                                                      Levy(dim))
                    X2 = transform(X2, dim)
                    particles_X2 = single_get_ObjectFunction(X2, dim, x, y, z)
                    if dominates(particles_X2, particles_F[i]):
